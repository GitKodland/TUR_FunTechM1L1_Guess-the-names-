...
export class UI {
  ...
  /* ---------- Переключение окон ---------- */
  toSetup() { this.winQuiz.classList.add("hidden"); this.winSetup.classList.remove("hidden"); }
  toQuiz()  { this.winSetup.classList.add("hidden"); this.winQuiz.classList.remove("hidden"); }
  ...
  /* ---------- Окно 2: drag & drop ---------- */
  // Строим сетку слотов (эмодзи) и пул карточек (имена)
  ...
  // Подсветка результата слотов после проверки
  markResults(results) {
    // results: [{slot, correct}]
  ...
  showResult(allOk, okCount, total) {
    ...
    this.resultTitle.textContent = allOk ? "Круто!" : "Есть ошибки";
    this.resultText.textContent  = allOk
      ? "Все ответы верные!"
      : `Правильных: ${okCount}/${total}`;
    this.resultModal.classList.remove("hidden");
  }

  // Обновление содержания подсказки:
  // mode = "setup" — общий текст, "quiz" — показать пары "эмодзи — имя"
